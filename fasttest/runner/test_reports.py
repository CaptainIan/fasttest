...
import json
import copy
from fasttest.common import Dict, DictEncoder, gvar
...
class TestReports(object):
    ...
    def __init__(self, testresult = None) -> None:
        ...
        self.summary = {
            ...
            'type': self._detect_type(testresult.result)
        }
        self.test_cases = []
    ...
    def _detect_type(self, results: list) -> str:
        """根据用例类标记自动识别测试类型"""
        for result in results:
            if getattr(result, 'is_api', False):
                return 'API'
        return 'UI'
